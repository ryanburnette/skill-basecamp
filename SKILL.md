...
### Show All Todos by Todolist

When user asks for todos, show all todos organized by todo list with assignee and due date:

```bash
# 1. Get all todolists for the project
basecamp todolists list --in <project> --json

# 2. For each todolist, get its todos
for list_id in $(basecamp todolists list --in <project> --jq '.data[].id'); do
  basecamp todos list --list "$list_id" --in <project> --md
done
```

Or use a script for faster execution:

```bash
# Get all todolists with their todos in one pass
basecamp todolists list --in <project> --json | jq -r '.data[] | .id' | while read list_id; do
  echo "## $(basecamp todolists show "$list_id" --in <project> --jq '.data.name')"
  basecamp todos list --list "$list_id" --in <project> --md
  echo ""
done
```

**Rule:** Always display all todos organized by todo list. Include assignee and due date for each.
...
When creating a new repo for Basecamp project integration:
...
tar -xzf /tmp/basecamp.tar.gz -C ~/.local/opt/basecamp-v${VER}/bin
ln -sf ~/.local/opt/basecamp-v${VER}/bin/basecamp ~/.local/bin/basecamp
...
```bash
basecamp config init
basecamp config set project_id <PROJECT_ID>
basecamp config set account_id <ACCOUNT_ID>
basecamp config trust
```

This creates `.basecamp/config.json`. Commit it to git.
...
project-repo/
├── .basecamp/
│   └── config.json     # project_id + account_id (committed)
├── bin/                # scheduled scripts (run on container)
├── .gitignore
├── AGENTS.md           # project context and command reference
└── opencode.json
```
